...
async function resolveEnsEncryptionKey(
	...
	const record = await client.getTextRecord({ name: ensName, key: ENS_TEXT_KEY });
	...
	if (colonIdx === -1) {
		throw new Error(`Malformed encryption key record for ${ensName}: ${record}`);
	}
	...
	if (curveStr === "secp256k1") {
		return { pubKey: hexToBytes(hexKey.replace("0x", "")), curve: CurveId.SECP256K1 };
	}
	if (curveStr === "x25519") {
		return { pubKey: hexToBytes(hexKey.replace("0x", "")), curve: CurveId.X25519 };
	}
